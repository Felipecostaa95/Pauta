...
def detect(conn, day, cfg, markets, db, categorias=None):
    """categorias: config global categorias_destacadas de config.yaml, NO
    cfg["spike"]. La exclusión dura (config.yaml -> excluir) NO pasa por acá:
    se aplica antes, por NOTA individual, al armar los vínculos item-entidad
    en run.py (ver tags.filter_excluded_items) — así un tema con una sola nota
    de gaming entre cinco no se descarta entero, se queda con las otras
    cuatro y menos volumen. Para cuando los candidatos llegan a este loop, las
    notas excluidas ya no cuentan en `daily` y por lo tanto tampoco en `vol`."""
